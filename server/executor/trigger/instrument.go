...
func (t *instrumentedTriggerer) Trigger(ctx context.Context, test test.Test, opts *TriggerOptions) (Response, error) {
	...
	tracestate, err := trace.ParseTraceState("quality-trace=true")
	if err != nil {
	...
	triggerSpan.SpanContext().TraceState().Insert("quality-trace", "true")
	...
	attrs := []attribute.KeyValue{
		attribute.String("quality-trace.run.trigger.trace_id", tid.String()),
		attribute.String("quality-trace.run.trigger.span_id", sid.String()),
		attribute.String("quality-trace.run.trigger.test_id", string(test.ID)),
		attribute.String("quality-trace.run.trigger.type", string(t.triggerer.Type())),
	}

	if err != nil {
		span.RecordError(err)
		attrs = append(attrs, attribute.String("quality-trace.run.trigger.error", err.Error()))
	}
