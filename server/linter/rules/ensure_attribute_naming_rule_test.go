...
func TestEnsureAttributeNamingRule(t *testing.T) {
	...
	t.Run("attribute named after namespace", func(t *testing.T) {
		trace := traceWithSpans(
			spanWithAttributes("http", map[string]string{"quality-trace.span": "POST"}),
		)
		...
		assert.False(t, result.Passed)
		assert.Len(t, result.Results, 1)
		assert.Equal(t, `Attribute "quality-trace.span" uses the same name as an existing namespace in the same span`, result.Results[0].Errors[0].Description)
	})
}
