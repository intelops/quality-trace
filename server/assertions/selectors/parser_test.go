...
func TestParserSingleSpanMultipleAttributes(t *testing.T) {
	expression := "span[service.name=\"Pokeshop\" quality-trace.span.type=\"http\"]"

	selector := parseExpressionOrFail(t, expression)

	assert.Equal(t, "service.name", selector.SpanSelectors[0].Filters[0].Property)
	assert.Equal(t, "\"Pokeshop\"", *selector.SpanSelectors[0].Filters[0].Value.String)
	assert.Equal(t, "quality-trace.span.type", selector.SpanSelectors[0].Filters[1].Property)
...
func TestParserSingleSpanWithNthChild(t *testing.T) {
	expression := "span[quality-trace.span.type=\"http\"]:nth_child(2)"

	selector := parseExpressionOrFail(t, expression)

	assert.Equal(t, "quality-trace.span.type", selector.SpanSelectors[0].Filters[0].Property)
	assert.Equal(t, "=", selector.SpanSelectors[0].Filters[0].Operator)
...
func TestParserWithSpanHierarchy(t *testing.T) {
	expression := "span[service.name=\"Pokeshop\"] span[quality-trace.span.type=\"http\"]"

	selector := parseExpressionOrFail(t, expression)

	assert.Equal(t, "service.name", selector.SpanSelectors[0].Filters[0].Property)
	assert.Equal(t, "quality-trace.span.type", selector.SpanSelectors[0].ChildSelector.Filters[0].Property)
...
func TestParserWithMultipleSpans(t *testing.T) {
	expression := "span[quality-trace.span.type=\"http\"], span[quality-trace.span.type=\"grpc\"]"
	...
	assert.Equal(t, "quality-trace.span.type", selector.SpanSelectors[1].Filters[0].Property)
...
func TestParserWithMultipleSpansAndHierarchy(t *testing.T) {
	expression := "span[quality-trace.span.type=\"http\"], span[service.name=\"Pokeshop\"] span[quality-trace.span.type=\"grpc\"]"
	...
	assert.Equal(t, "quality-trace.span.type", selector.SpanSelectors[1].ChildSelector.Filters[0].Property)
