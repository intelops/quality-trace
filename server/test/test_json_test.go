...
func TestSpecV1(t *testing.T) {
	...
	[
		{
			"Key": "span[quality-trace.span.type=\"general\" name=\"Qualitytrace trigger\"]",
			"Value": {
				"Name": "my check",
				"Assertions": [
	...
	assert.Equal(t, test.SpanQuery("span[quality-trace.span.type=\"general\" name=\"Qualitytrace trigger\"]"), testObject.Specs[0].Selector)
...
func TestV1WithEmptySelector(t *testing.T) {
	specsJSONWithEmptySelector := `[
		{
			...
			"Value": {
				"Name": "DURATION_CHECK",
				"Assertions": ["attr:quality-trace.span.duration < 2s"]
			}
		},
		{
			"Key": "span[quality-trace.span.type=\"database\"]",
			"Value": {
				"Name": "All Database Spans: Processing time is less than 100ms",
				"Assertions": ["attr:quality-trace.span.duration < 100ms"]
			}
		}
	]`
	testObject := test.Test{}
	...
	assert.Len(t, testObject.Specs[0].Assertions, 1)
	assert.Equal(t, test.Assertion("attr:quality-trace.span.duration < 2s"), testObject.Specs[0].Assertions[0])

	assert.Equal(t, test.SpanQuery("span[quality-trace.span.type=\"database\"]"), testObject.Specs[1].Selector)
	assert.Equal(t, "All Database Spans: Processing time is less than 100ms", testObject.Specs[1].Name)
	assert.Len(t, testObject.Specs[1].Assertions, 1)
	assert.Equal(t, test.Assertion("attr:quality-trace.span.duration < 100ms"), testObject.Specs[1].Assertions[0])
}
