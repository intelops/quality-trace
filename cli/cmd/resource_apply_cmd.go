...
func (p applyParameters) Validate(cmd *cobra.Command, args []string) []error {
	...
	if p.DefinitionFile == "" && (p.GitRepo == "" || p.GitUsername == "" || p.GitToken == "" || p.RepoName == "" || p.Branch == "") {
		errors = append(errors, paramError{
			Parameter: "file/git parameters",
			Message:   "Either a definition file or Git parameters (GitRepo, GitUsername, GitToken, RepoName, Branch) must be provided",
		})
	}
