...
func Exec(t *testing.T, qualitytraceSubCommand string, options ...ExecOption) *command.ExecResult {
	...
	qualitytraceCommand := config.GetConfigAsEnvVars().QualitytraceCommand
	qualitytraceSubCommands := strings.Split(qualitytraceSubCommand, " ")

	if config.GetConfigAsEnvVars().EnableCLIDebug {
		return runQualitytraceAsInternalCommand(t, qualitytraceCommand, qualitytraceSubCommands)
	}
...
func runQualitytraceAsInternalCommand(t *testing.T, qualitytraceCommand string, qualitytraceSubCommands []string) *command.ExecResult {
