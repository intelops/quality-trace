...
module "quality-trace_alb_security_group" {
  ...
  ingress_with_cidr_blocks = [
    {
      ...
      to_port     = 11633
      protocol    = "tcp"
      description = "HTTP access for Qualitytrace"
...
resource "aws_lb" "quality-trace-alb" {
  name               = "quality-trace-alb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [module.quality-trace_alb_security_group.security_group_id]
...
// INTERNAL ALB
module "internal_quality-trace_alb_security_group" {
  source  = "terraform-aws-modules/security-group/aws"
  version = "~> 4.0"
...
resource "aws_lb" "internal_quality-trace_alb" {
...
module "quality-trace_ecs_service_security_group" {
  source  = "terraform-aws-modules/security-group/aws"
  version = "~> 4.0"

  name        = "quality-trace_ecs_service_security_group"
