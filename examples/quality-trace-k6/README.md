...
This repository objective is to show how you can configure your Qualitytrace to run alongside your k6 load tests against an instrumented service.

<!--For more detailed information about the K6 Qualitytrace Binary take a look a the [docs](https://docs.tracetest.io/tools-and-integrations/integrations/k6).-->
...
4. Test if it works by running: `quality-trace run test -f tests/test.yaml`. This will create and run a test with trace id as trigger
5. In as separate folder outside of the the Qualitytrace repo, build the k6 binary with the extension by using `xk6 build v0.42.0 --with github.com/kubeshop/xk6-quality-trace`
6. Now you are ready to run your load test, you can achieve this by running the following command: `path/to/binary/k6 run import-pokemon.js -o xk6-quality-trace`
7. After the load test finishes you should be able to see an output like the following:

  ```bash
./k6 run quality-trace/examples/quality-trace-k6/import-pokemon.js -o xk6-quality-trace
...
[Request=POST - http://localhost:8081/pokemon/import, TraceID=dc0718a7e2aca9de30955b5203b162a7, RunState=FINISHED FailingSpecs=true, QualitytraceURL= http://localhost:3000/test/kc_MgKoVR/run/273] 
...
[Request=POST - http://localhost:8081/pokemon/import, TraceID=dc0718c9d2aca9de3044a794f7248eab, RunState=FINISHED FailingSpecs=false, QualitytraceURL= http://localhost:3000/test/kc_MgKoVR/run/271] 
  ...
  ERRO[0017] a panic occurred during JS execution: Qualitytrace: 5 jobs failed
  ```
