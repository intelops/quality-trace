resource "aws_ecs_task_definition" "quality-trace" {
  ...
  task_role_arn            = aws_iam_role.quality-trace_task_role.arn
...
resource "aws_ecs_service" "quality-trace_service" {
  name            = "${local.name}-service"
  cluster         = aws_ecs_cluster.quality-trace_cluster.id
  task_definition = aws_ecs_task_definition.quality-trace.arn
  desired_count   = 1
  launch_type     = "FARGATE"

  load_balancer {
    target_group_arn = aws_lb_target_group.quality-trace_tg.arn
...
resource "aws_lb_target_group" "quality-trace_tg" {
  name        = "${local.name}-tg"
  port        = 11633
  protocol    = "HTTP"
  vpc_id      = module.network.vpc_id
...
resource "aws_lb_listener" "quality-trace_alb_listener" {
  load_balancer_arn = aws_lb.quality-trace_alb.arn
  port              = "11633"
  protocol          = "HTTP"
