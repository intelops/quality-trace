# Running Qualitytrace with Tempo

:::note
[Check out the source code on GitHub here.](https://github.com/intelops/quality-trace/tree/main/examples/quick-start-tempo-nodejs)
:::
...
## Sample Node.js App with Tempo, OpenTelemetry and Qualitytrace
...
The `docker-compose.yaml` file, `collector.config.yaml`, `quality-trace-provision.yaml`, and `quality-trace-config.yaml` in the `quality-trace` directory are for the setting up Qualitytrace, Tempo, and the OpenTelemetry Collector.
...
All `services` in the `docker-compose.yaml` are on the same network and will be reachable by hostname from within other services. For example, `tempo:4317` in the `collector.config.yaml` will map to the `tempo` service, where the port `4317` is the port where Tempo accepts traces. And, `tempo:9095` in the `quality-trace-provision.yaml` will map to the `tempo` service and port `9095` where Qualitytrace will fetch trace data from Tempo.
...
services:
  ...
  tempo:
    ...
    volumes:
      - ./quality-trace/tempo.config.yaml:/etc/tempo.yaml

```

Qualitytrace depends on Postgres, Tempo and the OpenTelemetry Collector. All three services require config files to be loaded via a volume. The volumes are mapped from the root directory into the `quality-trace` directory and the respective config files.
...
The `quality-trace-config.yaml` file contains the basic setup of connecting Qualitytrace to the Postgres instance and defining the trace data store and exporter. The exporter is set to the OpenTelemetry Collector.
...
The `quality-trace-provision.yaml` file defines the trace data store, set to Tempo, meaning the traces will be stored in Tempo and Qualitytrace will fetch them from Tempo when running tests.

How does Qualitytrace fetch traces?

Qualitytrace uses `tempo:9095` to connect to Tempo and fetch trace data.
