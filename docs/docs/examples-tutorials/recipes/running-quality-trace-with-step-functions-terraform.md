# Running Qualitytrace with AWS Step Functions, AWS X-Ray and Terraform
...
2. From the Terraform output, configure the [Qualitytrace CLI]<!--(https://docs.tracetest.io/cli/configuring-your-cli)--> to point to the public load balancer endpoint with `quality-trace configure --endpoint <tracetest_url>`.
