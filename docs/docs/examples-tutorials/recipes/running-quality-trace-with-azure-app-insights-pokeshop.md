# Running Qualitytrace with Azure App Insights (OpenTelemetry Collector & Pokeshop API)
...
## Pokeshop API with Azure App Insights and Qualitytrace
...
The `docker-compose.yaml` file, `quality-trace.provision.yaml`, and `quality-trace.config.yaml` in the `quality-trace` directory are for the setting up the Pokeshop API, Qualitytrace and OpenTelemetry Collector.
...
Qualitytrace depends on Postgres and the OpenTelemetry collector. Qualitytrace requires config files to be loaded via a volume. The volumes are mapped from the root directory into the `root` directory and the respective config files.
