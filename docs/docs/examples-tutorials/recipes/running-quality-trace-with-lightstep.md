# Running Qualitytrace With Lightstep
...
This is a simple sample app on how to configure the [OpenTelemetry Demo `v0.3.4-alpha`](https://github.com/open-telemetry/opentelemetry-demo) to use [Qualitytrace]<!--(https://tracetest.io/) -->for enhancing your E2E and integration tests with trace-based testing and [Lightstep](https://lightstep.com/) as a trace data store.
...
The `quality-trace-provision.yaml` file contains the data store setup. The data store is set to `lightstep` meaning the traces will be received by Qualitytrace OTLP API and stored in Qualitytrace itself.
